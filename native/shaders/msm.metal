...
kernel void bucket_accumulation(
    ...
    point_set_identity(buckets[bucket_global_index]);
    ...
    for (uint i = 0; i < num_entries; i++) {
        ...
        if (entry.bucket_index == bucket_index) {
            ...
            if (point_is_identity(buckets[bucket_global_index])) {
                point_copy(buckets[bucket_global_index], point_j);
            } else {
...
kernel void bucket_reduction(
    ...
    for (int i = config.buckets_per_window - 1; i >= 0; i--) {
        JacobianPoint bucket = buckets[bucket_offset + i];
    ...
    // Store window sum
    point_copy(window_sums[window_index], window_sum);
}
...
kernel void final_reduction(
    ...
    for (int w = config.num_windows - 1; w >= 0; w--) {
        ...
        // Add window sum
        JacobianPoint ws = window_sums[w];
